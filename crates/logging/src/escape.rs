//! # Parsing of escape seqences for Terminal Output.
//!
//! The parsing is done though a state machine enum [`EscapeMode`].
//! On start, the state machine is [`EscapeMode::None`] state.
//! When a `\x1B` char is encountered, the state changes to [`EscapeMode::ExpectOpenBrace`] and following charaters aren't printed.
//!
...
//! If a `[` was supplied, the state changes to [`EscapeMode::StartSeqence`].
//!
//! From [`EscapeMode::StartSeqence`], the state machine branches into:
...
//!
//! In [`EscapeMode::ForegroundColor`], the next char is passed to [`theme::get_color`] to set the forground color and
...
//! sate is changed to [`EscapeMode::FinishSequence`].
//!
//! In [`EscapeMode::ForegroundBrightColor`], the next char is passed to [`theme::get_bright_color`] to set the forground color and
//! sate is changed to [`EscapeMode::FinishSequence`].
//!
...
//! If a `;` is encountered, state is changed to [`EscapeMode::StartSeqence`].

use crate::theme;

/// State Machine for processing escape seqences.
#[derive(PartialEq, Eq)]
#[repr(u8)]
pub enum EscapeMode {
    None,
    ...
    pub fn process(&mut self, fg: &mut (u8, u8, u8), bg: &mut (u8, u8, u8), c: char) -> bool {
        match self {
            EscapeMode::ExpectOpenBrace if c == '[' => {
                *self = EscapeMode::StartSeqence;
            }

            EscapeMode::StartSeqence if c == '3' => {
                *self = EscapeMode::ForegroundColor;
            }
            EscapeMode::StartSeqence if c == '4' => {
                *self = EscapeMode::BackgroundColor;
            }
            EscapeMode::StartSeqence if c == '9' => {
                *self = EscapeMode::ForegroundBrightColor;
            }
            EscapeMode::StartSeqence if c == '1' => *self = EscapeMode::ExpectZero,
