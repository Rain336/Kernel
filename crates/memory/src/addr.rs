...
use super::page_table::{PageOffset, PageTableIndex, PageTableLevel};
...
/// All bits outside the 9-bit indexes and 12-bit page offset are sign extended, meaning they have to have the same bits as the higest valid bit.
/// Here an example of a 5-level address:
///
/// +---------+-----------+-----------+-----------+-----------+-----------+--------------+
...
impl VirtAddr {
    ...
    pub const fn page_table_index(self, level: PageTableLevel) -> PageTableIndex {
        PageTableIndex::new_truncate((self.0 >> 12 >> ((level.as_u8() - 1) * 9)) as u16)
    }
}
